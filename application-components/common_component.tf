  ...
  provisioner "local-exec" {
    command = <<EOT
      curl -s --request POST '${local.sumologic_api_endpoint}/v1/apps/${local.application_component_app_id}/install' \
          ...
          --data-raw '{ "name": "${local.application_component_app_name}", "description": "${local.application_component_app_description}", "destinationFolderId": "${sumologic_folder.root_apps_folder.id}"}}'
    EOT
  }
}
