...
resource "sumologic_field" "db_cluster" {
    count = length(local.database_engines_values) > 0 && local.has_any_nonkubernetes_deployments ? 1 : 0
    data_type  = "String"
...
resource "sumologic_field" "pod_labels_db_cluster" {
    count = length(local.database_engines_values) > 0 && local.has_any_kubernetes_deployments ? 1 : 0
    data_type  = "String"
