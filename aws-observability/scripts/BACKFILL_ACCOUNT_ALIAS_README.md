...
> **Note:** For automation/CI, set the `SUMO_ACCESS_KEY` environment variable instead of the interactive prompt.
...
| `--deploy-env` | Yes | Deployment environment (e.g., `au`, `ca`, `ch`, `de`, `eu`, `jp`, `fed`, `kr`, `us1`, or `us2`) |
| `--filename` | Step 2 only | Path to the edited CSV file |
| `--dry-run` | No, Step 2 only | Validate and show what would be updated without making API calls |
