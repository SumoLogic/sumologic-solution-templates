...
| <a name="input_auto_enable_logs_filters"></a> [auto\_enable\_logs\_filters](#input\_auto\_enable\_logs\_filters) | Enter regex for matching logGroups. Regex will check for the name. | `string` | `"apigateway|lambda|rds"` | no |
| <a name="input_auto_enable_logs_tags_filters"></a> [auto\_enable\_logs\_tags\_filters](#input\_auto\_enable\_logs\_tags\_filters) | Enter comma separated key value pairs for filtering logGroups using tags. | `string` | `null` | no |
...
| <a name="input_aws_resource_tags"></a> [aws\_resource\_tags](#input\_aws\_resource\_tags) | Map of tags to apply to all AWS resources provisioned through the test Source Module | `map(string)` | `{}` | no |
...
| <a name="input_executeTest5"></a> [executeTest5](#input\_executeTest5) | True - If you want to execute this TestCase | `bool` | `false` | no |
| <a name="input_executeTest6"></a> [executeTest6](#input\_executeTest6) | True - If you want to execute this TestCase | `bool` | `false` | no |
| <a name="input_metric_namespaces"></a> [metric\_namespaces](#input\_metric\_namespaces) | Enter comma separated metric namespaces. | `list(string)` | <pre>[<br/>  "AWS/ApplicationELB",<br/>  "AWS/ApiGateway",<br/>  "AWS/DynamoDB",<br/>  "AWS/Lambda",<br/>  "AWS/RDS",<br/>  "AWS/ECS",<br/>  "AWS/ElastiCache",<br/>  "AWS/ELB",<br/>  "AWS/NetworkELB",<br/>  "AWS/SQS",<br/>  "AWS/SNS",<br/>  "AWS/EC2"<br/>]</pre> | no |
| <a name="input_metrics_tag_filters"></a> [metrics\_tag\_filters](#input\_metrics\_tag\_filters) | Enter comma separated key value pairs for filtering metrics using tags. | <pre>list(object({<br/>      type      = string<br/>      namespace = string<br/>      tags      = list(string)<br/>    }))</pre> | `[]` | no |
...
| <a name="input_sumologic_environment"></a> [sumologic\_environment](#input\_sumologic\_environment) | Enter au, ca, ch, de, eu, fed, jp, kr, us1 or us2. For more information on Sumo Logic deployments visit https://help.sumologic.com/APIs/General-API-Information/Sumo-Logic-Endpoints-and-Firewall-Security | `string` | n/a | yes |
