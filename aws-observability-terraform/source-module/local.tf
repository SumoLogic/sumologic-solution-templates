locals {
  ...
  # Decide if source needs to be created 
  ...
  # Decide if source needs to be updated
  ...
  use_llf_log_source          = var.collect_cloudwatch_logs == "Lambda Log Forwarder"
  use_kf_logs_source          = var.collect_cloudwatch_logs == "Kinesis Firehose Log Source"
